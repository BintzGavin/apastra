...
#### 2. File Inventory
- **Create**: `promptops/harnesses/generate-starter-packs.sh` (Bash script to generate starter pack structures based on community-prompt-pack schema)
- **Modify**: `docs/status/EVALUATION.md` (Update domain status version)
- **Read-Only**: `docs/vision.md` (Expansion 5), `promptops/schemas/community-prompt-pack.schema.json`

#### 3. Implementation Spec
- **Harness Architecture**: A standalone bash script in `promptops/harnesses/` that will generate community starter packs directories. These packs include pre-built datasets, evaluators, suites, and prompt specs. The script creates the basic skeleton for 4 mentioned types: summarization, extraction, classification, and code review.
...
- **Dependencies**: Depends on `community-prompt-pack.schema.json`.
