cat << 'INNER_EOF' > .sys/plans/$(date +%Y-%m-%d)-CONTRACTS-CanarySuiteAndDriftReport.md
#### 1. Context & Goal
...
#### 2. File Inventory
- **Create**:
  - `promptops/schemas/canary-suite.schema.json` (Schema for canary test suites)
  - `promptops/schemas/drift-report.schema.json` (Schema for comparing canary results to baselines)
  - `promptops/validators/validate-canary-suite.sh` (Validator script)
  - `promptops/validators/validate-drift-report.sh` (Validator script)
- **Modify**: None
- **Read-Only**:
