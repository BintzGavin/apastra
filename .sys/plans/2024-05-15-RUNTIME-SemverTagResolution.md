...
- **Trigger**: `README.md` promises that Git-first resolution supports pinning by semver tags (e.g., `#semver:<range>`), but `promptops/resolver/git_ref.py` currently only supports exact commit SHAs or exact git tags via `git show`.
...
- **Resolver Architecture**: The resolver chain (local → workspace → git ref) will remain structurally the same. However, when the `GitRefResolver` receives a `pin` starting with `semver:`, it will not immediately run `git show`. Instead, it will:
  ...
  5. Execute `git show {selected_tag}:promptops/prompts/{prompt_id}.yaml` (or `.json`).
...
- **Manifest Format**: The manifest format remains unchanged. The `pin` field (string) in `consumption.yaml` will continue to store the ref, but now `semver:^1.0.0` will be a functionally supported value.
- **Pseudo-Code**:
  ```python
  def resolve(self, prompt_id, pin):
      ...
      # existing logic...
      try git show pin:promptops/prompts/prompt_id.yaml
      if success return parsed yaml
      try git show pin:promptops/prompts/prompt_id.json
...
#### 4. Test Plan
- **Verification**: Run a python test script that creates dummy git tags (`v1.0.0`, `v1.1.0`, `v2.0.0`), writes a manifest pinning `semver:^1.0.0`, and calls `resolve(prompt_id, "semver:^1.0.0")`.
...
- **Edge Cases**:
  - No matching tags in the repository for the given range.
  - Invalid semver string.
  - Shallow clone where tags are not fetched.
