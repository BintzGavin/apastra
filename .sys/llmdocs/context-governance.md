...
- Promotion s  enforce that a matching Approval State record with decision="approved" and checks_passed=true exists on the `promptops-artifacts` branch before generating a promotion record. A successful promotion record triggers a delivery target sync via `deliv er .yml`.
...
- Rulesets / Branch Protection: Enforces required status che cks  (e.g., `gate` check) on pull requests to the `main` branch, and immutability (no updates/deletions) on tags. Automated secret scanning runs on PRs modifying prompts or datasets to fail if secret keywords are found.
- Artifacts Branch Topology: A ll de rived, machine-generated artifacts (run artifacts, regression reports, and promotion records, and approval states) are isolated on the `promptops-artifacts` branch instead of the main branch to reduce repo bloat and avoid merge conflicts.

**Se c tion  B: File Tree**
- `.github/CODEOWNERS`
...
- `.github/wo rkflows/se cret-scan.yml`
...
- `prompt op s/policie s/deprecation.md`
...
**Section C: Policy Inv entory**
- `p romptops/policies/acceptable-use.md`: Acceptable use constraints.
...
- `promp top s/policies/a ppeals.md`: Moderation decision appeals process.
- `promptops/policies/federation.md`: Governance rules for cross-custodian trust and namespace resolution.
- `promptops/policies/mirroring.md`: Requirements and processes for establish ing read-only mir rors.
...
- `promptops/ delivery/prod-target.y aml`: Target type `github_pr`, channel `prod`
...
**Se ction F: CODEOWNERS Sum mary**
...
- `promptops/suite s/` @ apastra/evaluation- team
...
- `promptops/d atasets/` @apastra/contrac ts-team
...
- `derived-index/baseline s/` @a pastra/evaluation-tea m
