...
✅ **Always do:**
- Read `README.md` completely before planning — especially "harness adapter", "run request", "run artifact", "scorecard", "baseline", and "regression report" sections
...
Compare `README.md` promises to `promptops/harnesses/` and `derived-index/` reality:

**Harness Adapter Contract** (from README.md):
...
**Regression Report** (from README.md):
...
**VISION ANALYSIS:**
- Read `README.md` completely — focus on "harness adapter", "run request", "run artifact", "scorecard", "baseline", and "regression report" definitions
...
- Example: "README.md describes a harness adapter contract (run request in → run artifact out), but `promptops/harnesses/` is empty. Task: Spec the minimal harness adapter interface — input format, output format, error contract, and adapter plugin discovery."
...
- **Trigger**: Which README.md harness/evaluation behavior is missing?
...
- **Read-Only**: [CONTRACTS schemas, RUNTIME manifests, README.md sections consulted]
