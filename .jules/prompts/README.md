...
### Planning Phase
Planners compare the vision documented in `README.md` against repository reality.  
...
No two roles write to the same file. Shared files (e.g., `README.md`) are **read-only** for all roles.
...
- **`README.md`**: read-only for every role. No edits. If a vision gap requires README changes, flag it for human review.
